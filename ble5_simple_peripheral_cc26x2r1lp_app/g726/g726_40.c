	...
	g726_state *state_ptr)
{
	...
	i = quantize_g726(d, y, qtab_723_40, 15);	/* i = ADPCM code */

	dq = reconstruct_g726(i & 0x10, _dqlntab[i], y);	/* quantized diff */

	sr = (dq < 0) ? se - (dq & 0x7FFF) : se + dq; /* reconstructed signal */

	dqsez = sr + sez - se;		/* dqsez = pole prediction diff. */

	update_g726(5, y, _witab[i], _fitab[i], dq, sr, dqsez, state_ptr);
	...
	g726_state *state_ptr)
{
	...
	y = step_size_g726(state_ptr);	/* adaptive quantizer step size */
	dq = reconstruct_g726(i & 0x10, _dqlntab[i], y);	/* estimation diff. */
